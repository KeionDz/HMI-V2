...
interface DeleteLayerBlockedModalProps {
  layerNumber: number;
  activePalletsCount: number;
  onClose: () => void;
}

export function DeleteLayerBlockedModal({
  layerNumber,
  activePalletsCount,
  onClose,
}: DeleteLayerBlockedModalProps) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-950 p-6 shadow-2xl animate-in zoom-in-95 duration-200">
        <button
          type="button"
          onClick={onClose}
          aria-label="Close layer warning"
          title="Close"
        ...
        <div className="mb-5 flex items-start gap-3 pr-8">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm leading-relaxed text-zinc-400">
              Layer {layerNumber} has {activePalletsCount} active pallet
              {activePalletsCount === 1 ? "" : "s"}. Set all pallets in this
              layer to inactive before deleting it.
            </p>
          </div>
        </div>
