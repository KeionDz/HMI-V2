...
interface DeleteCameraConfirmationModalProps {
  cameraName?: string;
  onClose: () => void;
  onConfirm: () => void;
}

export function DeleteCameraConfirmationModal({
  cameraName,
  onClose,
  onConfirm,
}: DeleteCameraConfirmationModalProps) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-950 p-6 shadow-2xl animate-in zoom-in-95 duration-200">
        <button
          type="button"
          onClick={onClose}
          aria-label="Close delete camera confirmation"
          title="Close"
        ...
        <div className="mb-5 flex items-start gap-3 pr-8">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm leading-relaxed text-zinc-400">
              This will remove {cameraName ? cameraName : "this camera"} from the
              pallet.
            </p>
          </div>
        </div>
